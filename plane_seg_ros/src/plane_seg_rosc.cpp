...
class Pass{
  ...
  private:
    ...
    ros::Subscriber plane_cloud_sub_;
...
Pass::Pass(ros::NodeHandle node_):
  ...
  hull_cloud_pub_ = node_.advertise<sensor_msgs::PointCloud2>("/plane_seg_n1/hull_cloud", 10);
  hull_markers_pub_ = node_.advertise<visualization_msgs::Marker>("/plane_seg_n1/hull_markers", 10);
  hull_marker_array_pub_ = node_.advertise<visualization_msgs::MarkerArray>("/plane_seg_n1/hull_marker_array", 10);
  look_pose_pub_ = node_.advertise<geometry_msgs::PoseStamped>("/plane_seg_n1/look_pose", 10);
...
void Pass::preProcessFromFile(std::string sceneName){
  ...
  preprossesor.setFrame(fixed_frame_);
...
void Pass::planePointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr &msg){
  ...
  std::string cloudFrame = msg->header.frame_id;
  ...
  auto dt = std::chrono::duration_cast<std::chrono::milliseconds>(endT - startT);
...
int main( int argc, char** argv ){
  ...
  // app->preProcessFromFile("TUM/desk/labeled_cloud/1305031453.374112");
